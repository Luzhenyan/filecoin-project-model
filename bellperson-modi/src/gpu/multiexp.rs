...
fn calc_window_size(n: usize, exp_bits: usize, core_count: usize) -> usize {
    ...
    let lower_bound = (((exp_bits * n) as f64) / ((I * core_count) as f64)).ln();
    ...
    E: Engine,
{
    ...
    ((((mem as f64) * (1f64 - MEMORY_PADDING)) as usize)
        - (I * core_count * ((1 << MAX_WINDOW_SIZE) + 1) * proj_size))
        / (aff_size + exp_size)
}
    ...
    E: Engine,
{
        ...
        G: CurveAffine,
    {
        ...
        let bucket_buffer = self
            .program
            .create_buffer::<<G as CurveAffine>::Projective>(I * self.core_count * bucket_len)?;
        let result_buffer = self
            .program
            .create_buffer::<<G as CurveAffine>::Projective>(I * self.core_count)?;
...
*/
        ...
        G: CurveAffine,
    {
        ...
        let num_groups = calc_num_groups(self.core_count, num_windows);
        let bucket_len = 1 << window_size;
        info!("!!!lzy!!!window_size:{},num_window:{},num_groups:{},bucket_len:{},basesn:{}", window_size, num_windows, num_groups, bucket_len, n);
        // Each group will have `num_windows` threads and as there are `num_groups` groups, there will
        // be `num_groups` * `num_windows` threads in total.
        // Each thread will use `num_groups` * `num_windows` * `bucket_len` buckets.
        ...
        let result_bytes =
            I * self.core_count * std::mem::size_of::<<G as CurveAffine>::Projective>();
        ...
        let mut results = vec![<G as CurveAffine>::Projective::zero(); I * self.core_count];
        unsafe {
