    ...
    E: Engine,
{
        ...
        G: CurveAffine,
    {
        ...
        let exp_bits = exp_size::<E>() * 8;
        let window_size = calc_window_size(n/2 as usize, exp_bits, self.core_count);
        let num_windows = ((exp_bits as f64) / (window_size as f64)).ceil() as usize;
        let num_groups = calc_num_groups(self.core_count, num_windows);
        let bucket_len = 1 << window_size;
        ...
        let stream1 = rustacuda::stream::Stream::new(StreamFlags::NON_BLOCKING, None)?;
        let stream2 = rustacuda::stream::Stream::new(StreamFlags::NON_BLOCKING, None)?;
        let module = &self.module;

        assert_eq!(n, bases.len(), "n and bases missmatch");
        info!("!!!lzy!!! base buffer g write begin");
        let mut base_buffer_g = copy_to_device_buffer(&bases[0..n/2], &stream1)?;
        info!("!!!lzy!!! base buffer g1 write begin");
        let mut base_buffer_g1 = copy_to_device_buffer(&bases[n/2 + 1..n], &stream2)?;
               
        info!("!!!lzy!!! exp buffer g write begin");
        let mut exp_buffer_g = copy_to_device_buffer(&exps[0..n/2], &stream1)?;
        info!("!!!lzy!!! exp buffer g1 write begin");
        let mut exp_buffer_g1 = copy_to_device_buffer(&exps[n/2 + 1..n], &stream2)?;
        ...
        let bucket_bytes1 = 2
            ...
            * bucket_len
            * std::mem::size_of::<<G as CurveAffine>::Projective>();
        let mut bucket_buffer_g1 =
            unsafe { rustacuda::memory::DeviceBuffer::<u8>::uninitialized(bucket_bytes1) }?;
        let result_bytes1 =
            2 * self.core_count * std::mem::size_of::<<G as CurveAffine>::Projective>();
        let mut result_buffer_g1 =
            unsafe { rustacuda::memory::DeviceBuffer::<u8>::uninitialized(result_bytes1) }?;
        // Make global work size divisible by `LOCAL_WORK_SIZE`
        ...
        unsafe {
                ...
                module.G1_bellman_multiexp<<<global_work_size as u32,
                                             LOCAL_WORK_SIZE as u32,
                                             0, stream2>>>
                    (base_buffer_g1.as_device_ptr(),
                     bucket_buffer_g1.as_device_ptr(),
                     result_buffer_g1.as_device_ptr(),
                     exp_buffer_g1.as_device_ptr(),
                     n as u32,
                     num_groups as u32,
                     num_windows as u32,
                     window_size as u32
        ...
        unsafe {
            ...
            info!("!!!lzy!!! result read begin");
            result_buffer_g.async_copy_to(bresults, &stream1)?;
            info!("!!!lzy!!! result read end");
            rustacuda::memory::DeviceBuffer::drop(result_buffer_g);
        }
        let mut results1 = vec![<G as CurveAffine>::Projective::zero(); 2 * self.core_count];
        unsafe {
            let bresults1: &mut [u8] = std::slice::from_raw_parts_mut(
                results1.as_mut_ptr() as *mut _ as *mut u8,
                result_bytes1,
            );
            //info!("!!!lzy!!! result1 read begin");
            info!("!!!lzy!!! result1 read begin");
            result_buffer_g1.async_copy_to(bresults1, &stream2)?;
            info!("!!!lzy!!! result1 read end");
        }
        ...
        let mut acc1 = <G as CurveAffine>::Projective::zero();
        let mut bits1 = 0;
        for i in 0..num_windows {
            let w = std::cmp::min(window_size, exp_bits - bits1);
            for _ in 0..w {
                acc1.double();
            }
            for g in 0..num_groups {
                acc1.add_assign(&results1[g * num_windows + i]);
            }
            bits1 += w; // Process the next window
        }
        acc.add_assign(&acc1);
        Ok(acc)
    } else if TypeId::of::<G>() == TypeId::of::<E::G2Affine>() {
    ...
    E: Engine,
{
        ...
        <G as groupy::CurveAffine>::Engine: crate::bls::Engine,
    {
        ...
        crate::multicore::THREAD_POOL.install(|| {
            ...
            let results = if n > 0 {
                bases
                    .par_chunks(chunk_size*2)
                    .zip(exps.par_chunks(chunk_size*2))
                    .zip(self.kernels.par_iter_mut())
                    .map(|((bases, exps), kern)| -> Result<<G as CurveAffine>::Projective, GPUError> {
                        let mut acc = <G as CurveAffine>::Projective::zero();
                        for (bases, exps) in bases.chunks(kern.n*2).zip(exps.chunks(kern.n*2)) {
