...
const MAX_LOG2_LOCAL_WORK_SIZE: u32 = 16; // 128
    ...
    E: Engine,
{
    ...
    pub fn radix_fft(&mut self, a: &mut [E::Fr], omega: &E::Fr, log_n: u32) -> GPUResult<()> {
        ...
        while log_p < log_n {
            ...
            info!("log_n:{}, log_p:{}, deg:{}, max_deg:{}", log_n, log_p, deg, max_deg);
            self.radix_fft_round(&src_buffer, &dst_buffer, log_n, log_p, deg, max_deg)?;
            log_p += deg;
            std::mem::swap(&mut src_buffer, &mut dst_buffer);
        }
        info!("----------------------------------total kernel run time: {:?}", start.elapsed());

        let start = Instant::now();
        src_buffer.read_into(0, a)?;
        self.program.finish()?;
        info!("----------------------------------gpu->cpu time: {:?}", start.elapsed());
