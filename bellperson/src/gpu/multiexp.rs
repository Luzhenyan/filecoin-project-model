...
const SOURCE_BIN: &[u8] = b"/root/go/src/github.com/filecoin-project/bellperson/src/gpu/multiexp/multiexp32.fatbin\0";
    ...
    E: Engine,
{
        ...
        G: CurveAffine,
    {
        ...
        let num_windows = ((exp_bits as f64) / (window_size as f64)).ceil() as usize;
        let num_groups = calc_num_groups(self.core_count, num_windows);
        let num_threads = 4352*4;
        let num_groups = num_threads / num_windows;
        let bucket_len = 1 << window_size;

        info!("basesize:{},expsize:{},bucketsize:{}",std::mem::size_of::<E::G1Affine>(),exp_size::<E>(),std::mem::size_of::<<G as CurveAffine>::Projective>());
        info!("n:{}, window_size:{}, num_windows:{}, num_groups:{}, bucket_len:{}", n, window_size, num_windows, num_groups, bucket_len);
        // Each group will have `num_windows` threads and as there are `num_groups` groups, there will
        // be `num_groups` * `num_windows` threads in total.
        // Each thread will use `num_groups` * `num_windows` * `bucket_len` buckets.
        ...
        //stream.synchronize()?;
        info!("----------------------------------cpu->gpu time: {:?}", start.elapsed());
        let start = Instant::now();
        ...
        let bucket_bytes = num_windows * num_groups
            * bucket_len
        ...
        let result_bytes =
            num_windows * num_groups * std::mem::size_of::<<G as CurveAffine>::Projective>();
        ...
        stream.synchronize()?;
        info!("----------------------------------kernel run time: {:?}", start.elapsed());

        let mut results = vec![<G as CurveAffine>::Projective::zero(); num_windows * num_groups];
        unsafe {
...
fn copy_to_device_buffer<T>(
    ...
    unsafe {
        ...
        info!("buffer_size:{}MB", bytes_len as f32 / 1024.0/ 1024.0);
        info!("each_size:{}B", bytes_len as f32 / 8000000.0);
